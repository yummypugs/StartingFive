    ...
    def get(self, name):
        ...
        if name < self.key:
            if self.left_child is None:  # this line is used to present an error message, if left key of node is empty then the get key is not present in the tree
                return str(
                    name) + " Not Found"  # returns the error message that, the key we are looking for is not present in the tree
            return self.left_child.get(
                name)  # a recursion function which takes left_child key as node and reruns the get function with left_child key as base
        # if node key less than the input string key ( from get function ), then this section will be executed, ( pls note key is string datatype, so comparison will be based on their alphabatical order )
            ...
            if self.right_child is None:  # this line is used to present an error message, if right key of node is empty then the get key is not present in the tree
                return str(
                    name) + " Not Found"  # returns the error message that, the key we are looking for is not present in the tree
            return self.right_child.get(
                name)  # a recursion function which takes left_child key as node and reruns the get function with left_child key as base
        else:
            return str(
                self.value)  # if none of the if and else if conditions stated above are true, then the node key we are comparing is the get key value we are looking for, and so the algorithm will return the corresponding value of the get key
